...
int allOddBits(int x) {
  int val = 0xAA; // 8bit
  int aval = (val << 8) + val; // 16bit
  int bval = (aval << 16) + aval; // 32bit
  int equal = (x & bval)^bval;
  return !equal;
}
/* 
 * negate - return -x 
 ...
 *   Rating: 3 
 * key:要获取全0 or 全1 
 * 只有在all zero 与all one的情况下，
 * 逻辑表达式的值才与位表达式有相同的行为
 */
int conditional(int x, int y, int z) {
  int cond = !!x;
  int flag = ~cond + 1;//获取all one与all zero
  int eq1 = ((flag) & y);
  int eq2 = ((~flag) & z);
  return (eq1 | eq2);
}
 ...
 *   Max ops: 24
 *   Rating: 3
 y - x的结果进行推理得出，注意可能存在溢出！
 */
int isLessOrEqual(int x, int y) {
  int sign = (y + (~x) + 1) >> 31; //  x <= y : 0

  // sx sy >= 0
  int sx = x >> 31;
  int sy = y >> 31;
  // x>0 y <0  x-y>=0
  int case1 =  (!sx) & sy;// x>0 y <0
  int case2 = sx & (!sy);// x<0 y>0
...
/*这个问题的本质是找最左侧的一个1（postive number） find it 在顺序条件下二分是好的。*/
...
// printf("sign = %d,exponent = 0x%x,fraction = 0x%x\n",fsign,fexponent,ffraction);
// printf("uf = 0x%x\n",uf);
  /* normalized number */
...
int floatFloat2Int(unsigned uf) {
  ...
  int E = Exp - 127;
  int result;
// printf("sign = %d,Exp = 0x%x,fraction = 0x%x\n",sign,Exp,fraction);
// printf("uf = 0x%x\n",uf);
  fraction |= 0x00800000;
  if (E < 0)
    return 0;
