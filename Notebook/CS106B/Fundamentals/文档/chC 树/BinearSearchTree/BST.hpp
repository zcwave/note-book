...
private:
    ...
    Node* root{};
    static const size_t knot_found = -1;
...
public:
    ...
    ValueType get(KeyType&& key) {
        ...
        if (t == nullptr) {
            insertNode(root, std::forward<KeyType&&>(key), std::forward<ValueType>(knot_found));
            return knot_found;
        }
        // throw std::out_of_range("GET: NOT FOUND IT.");
        return t->val;
    }
...
private:
    ...
    std::optional<Node*> findNode(Node* t, KeyType&& key) {
